...
export default function ParentCounsellingScreen() {
  const { user } = useAuthStore();
  const { colors } = useAppTheme();

  const wardUsn = user?.wardUsn ?? user?.usn ?? '';
  const wardName = useMemo(() => {
    const student = mockStudents.find(s => s.usn === wardUsn);
    return student?.name ?? 'Ward';
  }, [wardUsn]);

  const sessions = useMemo(() => getStudentCounsellingSessions(wardUsn), [wardUsn]);
