  ...
  return (
    <View className="flex-1" style={{ backgroundColor: colors.bg }}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <Card className="mb-3">
            <View className="border-l-4 pl-3" style={{ borderLeftColor: colors.accentStudent }}>
              <Text className="font-bold text-base" style={{ color: colors.text }}>{item.title}</Text>
              {item.targetRole && (
                <View className="self-start rounded px-2 py-0.5 mt-1" style={{ backgroundColor: colors.accentStudent + '30' }}>
                  <Text className="text-[10px] uppercase tracking-wider" style={{ color: colors.accentStudent }}>{item.targetRole}</Text>
                </View>
              )}
            </View>
