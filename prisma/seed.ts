...
function workingDays(daysBack: number): Date[] {
  const dates: Date[] = [];
...
function randomMark(min: number, max: number): number {
...
async function main() {
  ...
  // ── Passwords ───────────────────────────────────────────────────────────────
  const [studentHash, facultyHash, parentHash, hodHash, principalHash] =
    await Promise.all([
      bcrypt.hash('student123', SALT_ROUNDS),
      bcrypt.hash('faculty123', SALT_ROUNDS),
      bcrypt.hash('parent123', SALT_ROUNDS),
      bcrypt.hash('hod123', SALT_ROUNDS),
      bcrypt.hash('principal123', SALT_ROUNDS),
    ]);
  ...
  const facultyUsers: { id: number; profileId: number }[] = [];
  ...
  for (const sp of studentProfiles) {
    ...
    const absenceIndices = new Set<number>();
    ...
    for (const subject of sectionSubjects) {
      ...
      const mapping = mappings.find(
        (m) =>
          subjects[m.si].id === subject.id && m.section === sp.section
      );
      ...
      for (let di = 0; di < dates.length; di++) {
        ...
        if ([6, 7].includes(subjects.indexOf(subject))) {
          ...
          if (dateDay !== 2 && dateDay !== 4) continue;
        }
  ...
  for (const sp of studentProfiles) {
    for (const subject of subjects) {
      ...
      for (let cie = 1; cie <= 3; cie++) {
        if (cie === 2 && Math.random() > 0.7) continue;
        if (cie === 3 && Math.random() > 0.4) continue;
        marksData.push({
          studentProfileId: sp.profileId,
          subjectId: subject.id,
          cieNumber: cie,
    ...
    data: [
      {
        ...
        content:
          'CIE-2 examinations for 5th semester will be conducted from 15th to 22nd of this month. Students are advised to prepare accordingly. Timetable will be displayed on the notice board.',
        targetRole: null,
        departmentId: null,
      ...
      },
      {
        ...
        content:
          'Students with attendance below 75% in any subject will not be permitted to appear for the end-semester examination. Please regularize your attendance immediately.',
      ...
      },
      {
        ...
        content:
          'An industrial visit has been arranged to Infosys Mysuru campus on 28th of this month. Interested students from CSE 5th semester may register with their class representatives by this Friday.',
        targetRole: null,
        departmentId: cseDept.id,
        postedByUserId: cseHod.id,
        isActive: true,
      },
      {
        ...
        content:
          'A 3-day Faculty Development Programme on "AI/ML Applications in Education" is scheduled next week. All faculty members are requested to register on the college portal.',
        targetRole: Role.FACULTY,
        departmentId: null,
        postedByUserId: cseHod.id,
        isActive: true,
      },
      {
        ...
        content:
          'The college will remain closed on 26th January on account of Republic Day. Regular classes will resume from 27th January.',
        targetRole: null,
        departmentId: null,
        postedByUserId: cseHod.id,
        isActive: true,
